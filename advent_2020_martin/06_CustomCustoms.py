...
"""  # Cheeky blank line to make the magic work
...
"""  # Another cheeky blank line


def part_one(form):
    answers = form.splitlines()
    total_yesses = 0
    group_answer = ""
    for answer in answers:
        if answer == "":  # Blank entry means end of group, save and reset group_answer
            total_yesses += len(group_answer)
            group_answer = ""
            continue
        for char in answer:
            if char not in group_answer:
                group_answer += char

    print("*"*96)
    print("PART ONE")
    print("{} total yesses.".format(total_yesses))
...
def part_two(form):
    ...
    for answer in answers:
        if answer == "":  # Blank entry means end of group, save and reset group_answer
            unanimous_yesses += len(group_answer)
            group_answer = "-1"
            continue
        if group_answer == "-1":
            group_answer = answer
        else:
            group_answer = set(group_answer).intersection(answer)
            group_answer = ''.join(group_answer)

    print("*"*96)
    print("PART TWO")
    print("{} total unanimous yesses.".format(unanimous_yesses))
