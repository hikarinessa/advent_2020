...
cm = "cm"
inch = "in"
min_max_height = {cm:[150, 193], inch:[59, 76]}


def validate_year(value: str, min_val, max_val):
    int_value = int(value)
    if len(value) != 4:
        return False
...
def validate_height(value: str):
    metric = value[-2::]
    if metric not in min_max_height.keys():
        return False
    height = int(value[:-2:])
    min_height, max_height = min_max_height[metric]
...
def validate_hair_color(value: str):
...
def validate_entry(entry: str):
    field, colon, value = entry.partition(":")
...
def part_one(ignore):
    ...
    print("*"*96)
    print("PART ONE")
    print("Found {} valid passports.".format(valid_passports))
